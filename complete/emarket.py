...
def main():
    ...
    if sku_value != None:
        ...
        try:
            ...
            time.sleep(1)
            driver.find_element_by_class_name("product-intro__description-head").click()
            descriptions = driver.find_element_by_class_name("product-intro__description-table").get_attribute('innerHTML')
            ...
            # sku_really_price = sku_really_prices.replace("руб.","")
            # sku_really_price = sku_really_prices.replace("₪","")
            sku_really_price = sku_really_prices.replace("SR","")
            sku_id = insert_mysql_data(sku_name, sku_really_price, sku_value, descriptions)
            ...
            insert_sku_size_option_function(sku_id)
            sku_position = -1
...
def insert_mysql_data(sku_name, sku_really_price, sku_value, descriptions):
    sku_id = -1
    ...
    try:
        ...
        print("------------->sku_really_price", float(sku_really_price))
        sku_really_prices = float(sku_really_price)*1.25
        print('sku_really_prices-------->', sku_really_prices)
        mySql_insert_query = "INSERT INTO products (brand_id, slug, special_price, special_price_type, selling_price, sku, in_stock, is_active, created_at, updated_at) VALUES ('{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}', '{}')".format(25, sku_name, sku_really_prices, 'fixed', sku_really_prices, sku_value, 1, 1, currenttime, currenttime)
        ...
        sku_id = myresult[0][0]
        mySql_insert_query = "INSERT INTO product_translations (product_id, locale, `name`, `description`) VALUES ('{}', '{}', '{}', '{}')".format(sku_id, 'en_US', sku_name, descriptions)
        cursor.execute(mySql_insert_query)
        connection.commit()
        mySql_insert_query = "INSERT INTO product_translations (product_id, locale, `name`, `description`) VALUES ('{}', '{}', '{}', '{}')".format(sku_id, 'ar', sku_name, descriptions)
        cursor.execute(mySql_insert_query)
        connection.commit()

        mySql_insert_query = "INSERT INTO product_categories (product_id, category_id) VALUES ('{}', '{}')".format(sku_id, 127)
        cursor = connection.cursor()
        cursor.execute(mySql_insert_query)
        connection.commit()  
...
def upload_oneimage(sku_id, filename) :
    url = 'https://emarket.ps/single-upload.php'
    item = {"skuid": sku_id}
...
def insert_sku_size_function(sku_id, sku_position, sku_size_name):
    ...
    mySql_insert_query = "INSERT INTO option_value_translations (`option_value_id`, `locale`, `label`) VALUES ('{}', '{}', '{}')".format(myresult_id, 'en_US', sku_size_name)
    cursor = connection.cursor()
    cursor.execute(mySql_insert_query)
    connection.commit()
    mySql_insert_query = "INSERT INTO option_value_translations (`option_value_id`, `locale`, `label`) VALUES ('{}', '{}', '{}')".format(myresult_id, 'ar', sku_size_name)
    cursor = connection.cursor()
    cursor.execute(mySql_insert_query)
    connection.commit()
...
def insert_sku_size_option_function(sku_id):
    currenttime = datetime.datetime.now()
    connection = mysql.connector.connect(
         host="host ip",
            user="host user",
    ...
    product_name_en = "clothes measurements"
    product_name_ar = "قياسات الملابس"
    print('product_name---------->', product_name_en, product_name_ar)
    cursor = connection.cursor()
    ...
    mySql_insert_query = "INSERT INTO option_translations (`option_id`, `locale`, `name`) VALUES ('{}', '{}', '{}')".format(sku_id, 'en_US', product_name_en)
    cursor.execute(mySql_insert_query)
    connection.commit()
    mySql_insert_query = "INSERT INTO option_translations (`option_id`, `locale`, `name`) VALUES ('{}', '{}', '{}')".format(sku_id, 'ar', product_name_ar)
    cursor.execute(mySql_insert_query)
    connection.commit()
