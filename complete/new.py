...
def insert_mysql_data(sku_name, sku_really_price, sku_value):
    ...
    try:
        ...
        # print("->sku_value", sku_value)
        # cursor = connection.cursor()
        # sku_none_checking = cursor.execute("SELECT * FROM products WHERE sku = %s", (sku_value, ))
        
        # print("cursor.executets ", sku_none_checking)
        # if sku_none_checking != None:
        #     mySql_insert_query = "UPDATE products SET in_stock = %s, updated_at = %s WHERE sku = %s", (0, currenttime, sku_value)
...
def sku_value_none():
    print("Sorry, Sku doesn't in Site, Please try again ...")
